...
for i in range(1, opt.num_images+1):
    ...
    image = IMAGE(path_to_image=opt.data_dir, name_of_image=str(i), extension='.jpg')
    image.save_left_centre_right(opt.write_dir, n_H_expected=opt.required_height, n_W_expected=opt.required_width)
    image.fliplr()
    image.save_left_centre_right(opt.write_dir, n_H_expected=opt.required_height, n_W_expected=opt.required_width)

    image = IMAGE(path_to_image=opt.data_dir, name_of_image=i, extension='.jpg')
    image.flipud()
    image.save_left_centre_right(opt.write_dir, n_H_expected=opt.required_height, n_W_expected=opt.required_width)
    image.fliplr()
    image.save_left_centre_right(opt.write_dir, n_H_expected=opt.required_height, n_W_expected=opt.required_width)
    ...
    image.write_to_file(opt.write_dir+str(i)+'_warped.png')